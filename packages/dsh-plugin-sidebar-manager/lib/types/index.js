...
let PluginManagerGateway = (() => {
    ...
    return class PluginManagerGateway extends _classSuper {
        ...
        async setEnabled(request) {
            ...
            if (entry === undefined)
                return { ok: false, message: 'plugin entry not found: ' + request.entryId };
            if (entry.options.group)
                return { ok: false, message: 'plugin groups cannot be changed' };
            ...
            if (request.enabled && !entry.disabled)
                return { ok: true };
            if (desiredDisabled && entry.options.disabled === true)
                return { ok: true };
